...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, action_bound,offset):
        super(Actor, self).__init__()
        n_neurons = 256
        self.offset=offset
        self.act_bound = action_bound
    ...
    def forward(self, s, mean=False, with_prob=True):
        ...
        mu = self.mu(x)
        if mean:        
            action=torch.tanh(mu)* self.act_bound
            action[0][3] = action[0][3] + self.offset
            return action
            # return torch.tanh(mu)
        
        else:
            std = torch.clamp(self.log_std(x), -20, 1).exp()
            dist = torch.distributions.Normal(mu, std)
            action = dist.rsample()
            real_action = torch.tanh(action)
            real_action = real_action * self.act_bound
            real_action[0][3]=real_action[0][3]+self.offset
            if with_prob:
...
class SacAgent:
    def __init__(self, state_dim, action_dim,act_bound,offset, device):
        self.state_dim = state_dim
        self.action_dim = action_dim
        self.offset=offset
        self.device = device
        self.act_bound=np_to_tensor(act_bound,self.device)
        self.actor = opt_cuda(Actor(self.state_dim, self.action_dim,self.act_bound,self.offset), self.device)
