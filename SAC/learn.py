...
def learn(device, environment='Path_planning-v0', log=1):
    ...
    agent.actor.load_state_dict(torch.load('saves/Path_planning-v0835916.pth'))
    total_frames = 0
    ...
    while total_frames < pretrain_len:
        state = pretrained_paths[total_frames][0:24]
        next_state = pretrained_paths[total_frames][24:48]
        action = pretrained_paths[total_frames][48:55]
        reward = pretrained_paths[total_frames][55]
        done = pretrained_paths[total_frames][56]
        agent.remember(state, next_state, action, reward, done)
        total_frames += 1

    prev_frame = pretrain_len
    # for warm up
    threshold = pretrain_len*10
    maxstep = 4e3
    while total_frames < 2.1e6:
        state = env.reset()
        frame = 0
        ...
        while 1:
            ...
            if done:
                if total_frames >= threshold*10:
                    ...
                    if testres>1000 or testres==10:
                        torch.save(agent.actor.cpu().state_dict(), "saves/" + environment+str(total_frames - threshold) + ".pth")
                        agent.actor = opt_cuda(agent.actor, device)
                print('frames:', total_frames- threshold, 'done.   Interval:', total_frames - prev_frame)
                print("————————————————————————————————————————————————————")
...
if __name__ == '__main__':
    ...
    learn(device=args.gpu, environment=environment, log=args.log)
